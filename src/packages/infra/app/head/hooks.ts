...
const getUrl = (props: Props, settings: Settings) => {
  if (props.canonical) {
    ...
    return `${settings.siteUrl.replace(/\/$/, '')}/${props.canonical.replace(/^\//, '')}`;
  }
...
export const useHooks = (props: Props, settings: Settings) => {
  ...
  const image = props.image ?? settings.seo.blogImage ?? `https://og-image.vercel.app/${encodeURI(title)}.png?theme=light&md=0&fontSize=75px&images=https%3A%2F%2Fassets.zeit.co%2Fimage%2Fupload%2Ffront%2Fassets%2Fdesign%2Fnextjs-black-logo.svg`;
